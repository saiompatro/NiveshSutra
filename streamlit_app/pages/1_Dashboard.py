...
with lead_col:
    ...
    if signals:
        for signal in signals:
            ...
            row_left.markdown(
                f"**{signal['symbol']}**  \n"
                f"<span style='color:#8B949E;font-size:0.82rem'>Confidence {signal.get('confidence', 0) * 100:.0f}%</span>",
    ...
    if watchlist:
        for item in watchlist:
            ...
            change_color = "#23C55E" if change_pct >= 0 else "#EF4444"
            row_left, row_mid, row_right = st.columns([1.4, 1, 0.9], gap="medium")
            row_left.markdown(
                f"**{item['symbol']}**  \n"
                f"<span style='color:#8B949E;font-size:0.8rem'>{item['company_name']}</span>",
                unsafe_allow_html=True,
            )
            row_mid.markdown(
                f"<span style='font-size:1rem;color:#F0F6FC'>{item['price']:,.2f}</span>",
                unsafe_allow_html=True,
            )
            row_right.markdown(
