...
def render_signal_row(
    ...
    *,
    ...
    row[6].markdown(
        f"<span style='opacity:{opacity};color:#8B949E'>{get_position_size_hint(risk_profile, signal_row['signal']) or 'Observe only'}</span>",
        unsafe_allow_html=True,
    )

    if signal_row["symbol"] in accepted_symbols:
        row[7].markdown("<span style='color:#23C55E'>Tracked</span>", unsafe_allow_html=True)
    else:
    ...
    if st.session_state.get(f"accept_dialog_{signal_row['symbol']}"):
        with st.form(f"accept_form_{signal_row['symbol']}"):
            ...
            st.markdown(
                ...
                f"<span style='color:#8B949E;font-size:0.85rem'>Confidence {confidence:.0f}%</span>",
                unsafe_allow_html=True,
            )
            quantity = None
            price_input = None
...
if accepted:
    ...
    for item in accepted:
        ...
        row[2].markdown(f"<span style='color:#8B949E'>{pretty_date}</span>", unsafe_allow_html=True)
        with row[3]:
