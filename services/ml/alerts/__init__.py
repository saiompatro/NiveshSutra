...
def _check_signal_changes(
    sb: Any, today_str: str, yesterday_str: str
) -> list[dict]:
    """Compare today's signals with yesterday's and flag changes."""
    alerts: list[dict] = []
    ...
    today_map = (
        {r["symbol"]: r["signal"] for r in today_resp.data}
        if today_resp.data
        else {}
    )
    yesterday_map = (
        {r["symbol"]: r["signal"] for r in yesterday_resp.data}
            ...
            alerts.append(
                {
                    ...
                    "message": (
                        f"{symbol} signal changed from {old_signal} to {new_signal}."
                    ),
                    "symbol": symbol,
                    "is_read": False,
...
def _check_sentiment_shifts(
    ...
    today_map = (
        {r["symbol"]: r["avg_sentiment"] for r in today_resp.data}
        if today_resp.data
        else {}
    )
    yesterday_map = (
        {r["symbol"]: r["avg_sentiment"] for r in yesterday_resp.data}
        if yesterday_resp.data
        else {}
    )
            ...
            alerts.append(
                {
                    ...
                    "message": (
                        ...
                        f"(from {old_sent:.2f} to {new_sent:.2f}), "
                        f"turning {direction}."
                    ),
                    "symbol": symbol,
                    "is_read": False,
...
def _check_rebalance_drift(sb: Any) -> list[dict]:
    ...
    # Get the latest completed optimization per user
    opt_resp = (
    ...
    for opt in latest_opts:
        ...
        rec_weights = {
            r["symbol"]: r["recommended_weight"] for r in alloc_resp.data
        }
